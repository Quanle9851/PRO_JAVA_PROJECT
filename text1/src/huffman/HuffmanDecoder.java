...
public class HuffmanDecoder {
    ...
    private String convert8 (int aByte){
        ...
        int L= bitStr.length();
        if(L>8)bitStr=bitStr.substring(L-8,L);
        else while (bitStr.length()<8) bitStr = "0" + bitStr;
        return bitStr;
    }
    ...
    public void decode(){
        ...
        for (int i=0; i<zippedData.length-1;i++){
            ...
            if(decodingBitStr.length()>15){
                while (decodingBitStr.length()>15) temp.add((byte)(decodeSymbol()));
            }
        }
        ...
        int i;
        for (i=0; i<srcLen; i++) srcData[i]= temp.get(i).byteValue();
        decoded = true;
    }
